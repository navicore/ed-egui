...
impl TextBuffer {
    ...
    pub fn move_cursor_word_left(&mut self) {
        ...
        while self.cursor_pos > 0 && self.text[self.cursor_pos - 1..].starts_with(|c: char| c.is_whitespace()) {
            self.cursor_pos -= 1;
        }
    ...
    pub fn move_cursor_word_right(&mut self) {
        ...
        while self.cursor_pos < self.text.len() && !self.text[self.cursor_pos..].starts_with(|c: char| c.is_whitespace()) {
            self.cursor_pos += 1;
        }
